...
static int get_sw64_physical_address(CPUSW64State *env, target_ulong addr,
                                ...
                                int *pprot, target_ulong *page_size)
{
    CPUState *cs = CPU(sw64_env_get_cpu(env));
    target_ulong phys = 0;
    int prot = 0;
    int ret = MM_K_ACV;
    target_ulong page_offset = 0;
    target_ulong L1pte, L2pte, L3pte, pte;
...
do_pgmiss:
    ...
    L1pte = ldq_phys_clear(cs, pt + index * 8) & PTE_MASK;
    ...
    L2pte = ldq_phys_clear(cs, pt + index * 8) & PTE_MASK;
    ...
    L3pte = ldq_phys_clear(cs, pt + index * 8) & PTE_MASK;
    ...
    target_ulong page_granularity = (L3pte >> 5) & 0x3;
    if (page_granularity != 0) {
        pte = L3pte;
        *page_size = 1 << 23;
        page_offset = addr & 0x7fe000;
    } else {
        *page_size = TARGET_PAGE_SIZE;
        page_offset = 0;
        pte = ldq_phys_clear(cs, pt + index * 8) & PTE_MASK;
        if ((pte & PTE_VALID) == 0) {
            ret = MM_K_TNV;
            goto exit;
        }
...
#error page bits out of date
    ...
    phys = (pte >> pte_pfn_s << TARGET_PAGE_BITS) + page_offset;
...
do_pgmiss:
    fail = get_sw64_physical_address(env, address, 1 << access_type, mmu_idx,
                                    &phys, &prot, &page_size);
    if (unlikely(fail >= 0)) {
	if (probe) {
	    return false;
...
do_pgmiss:
    fail = get_sw64_physical_address(&cpu->env, addr, 1, mmu_index,
                                    &phys, &prot, &page_size);
done:
...
bool sw64_cpu_exec_interrupt(CPUState *cs, int interrupt_request)
{
    ...
    if (interrupt_request & CPU_INTERRUPT_IINM) {
        idx = EXCP_IINM;
        env->csr[IINM] = 1UL << 1;
        cs->interrupt_request &= ~CPU_INTERRUPT_IINM;
        goto done;
    }
...
{
    if ((index == C3_PRI_BASE) || (index == C4_PRI_BASE))
