...
#undef DEF_VAR
    ...
    for (i = 0; i < 31; i++) {
        cpu_std_ir[i] = tcg_global_mem_new_i64(
            tcg_env, offsetof(CPUSW64State, ir[i]), ireg_names[i]);
    }

    for (i = 0; i < 128; i++) {
        cpu_fr[i] = tcg_global_mem_new_i64(
            tcg_env, offsetof(CPUSW64State, fr[i]), freg_names[i]);
    }
    ...
    for (i = 0; i < 10; i++) {
        ...
        cpu_hm_ir[r] = tcg_global_mem_new_i64(
            tcg_env, offsetof(CPUSW64State, sr[i]), shadow_names[i]);
    }
#endif
}
...
{
    return ((tb_cflags(ctx->base.tb) & 1) ||
            ctx->base.singlestep_enabled || 1);
}
...
{
    TCGv mask = tcg_constant_i64((1UL << size) - 1);
    int i;
...
static inline void gen_load_mem(
    ...
    int ra, int rb, int32_t disp16, bool fp, uint64_t mask)
{
    TCGv tmp, addr, va;
    ...
    if (mask) {
        tcg_gen_andi_i64(tmp, addr, mask);
    }

    va = (fp ? cpu_fr[ra] : load_gir(ctx, ra));
    tcg_gen_qemu_load(va, addr, ctx->mem_idx);
    gen_helper_trace_mem(tcg_env, addr, va);
                             ...
                             int32_t disp13, uint16_t fn)
{
    ...
    case 0x4C:
        ...
        tcg_gen_ext32u_i64(tmp1, va);
        tcg_gen_shl_i64(vc, tmp1, tmp);
    ...
    case 0x5F:
        /* CASL */
        arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
        tcg_gen_atomic_cmpxchg_i64(vc, vb, va, vc, ctx->mem_idx, MO_TEUQ);
        break;
    case 0x60:
        /* MASKLB */
...
static void cal_with_imm_2(DisasContext *ctx, TCGv vc, TCGv va, int64_t disp,
                           uint8_t fn)
{
    TCGv_i64 t0 = tcg_constant_i64(disp);
...
static void cal_with_imm_3(DisasContext *ctx, TCGv vd, TCGv va, int32_t disp,
                           TCGv vc, uint8_t fn)
{
    TCGv_i64 vb = tcg_constant_i64(disp);
...
{
    tcg_gen_qemu_ld_i64(cpu_fr[t0], t1, memidx, MO_ALIGN_4 | MO_TEUQ);
    tcg_gen_mov_i64(cpu_fr[t0 + 32], cpu_fr[t0]);
    tcg_gen_mov_i64(cpu_fr[t0 + 64], cpu_fr[t0]);
    tcg_gen_mov_i64(cpu_fr[t0 + 96], cpu_fr[t0]);
}
...
static void gen_qemu_vldd(int t0, TCGv t1, int memidx)
{
    ...
    tcg_gen_qemu_ld_i64(cpu_fr[t0 + 32], t1, memidx, MO_TEUQ);
    tcg_gen_addi_i64(t1, t1, 8);
    tcg_gen_qemu_ld_i64(cpu_fr[t0 + 64], t1, memidx, MO_TEUQ);
    tcg_gen_addi_i64(t1, t1, 8);
    tcg_gen_qemu_ld_i64(cpu_fr[t0 + 96], t1, memidx, MO_TEUQ);
}
...
{
    tcg_gen_qemu_st_i64(cpu_fr[t0], t1, memidx, MO_ALIGN_4 | MO_TEUQ);
    tcg_gen_addi_i64(t1, t1, 8);
    tcg_gen_qemu_st_i64(cpu_fr[t0 + 32], t1, memidx, MO_TEUQ);
    tcg_gen_addi_i64(t1, t1, 8);
    tcg_gen_qemu_st_i64(cpu_fr[t0 + 64], t1, memidx, MO_TEUQ);
    tcg_gen_addi_i64(t1, t1, 8);
    tcg_gen_qemu_st_i64(cpu_fr[t0 + 96], t1, memidx, MO_TEUQ);
}
...
{
#ifndef CONFIG_USER_ONLY
    TCGv_i32 reg = tcg_constant_i32(rc + 32);
    gen_helper_fp_exc_raise(tcg_env, reg);
#endif
}
...
static void gen_fcvtdl(int rb, int rc, uint64_t round_mode)
{
    ...
    gen_helper_fcvtdl(cpu_fr[rc], tcg_env, cpu_fr[rb], tmp64);
    gen_fp_exc_raise(rc);
}
                            ...
                            uint64_t round_mode)
{
    ...
    tmp64 = tcg_temp_new_i64();
    tcg_gen_movi_i64(tmp64, round_mode);
    helper(cpu_fr[rc], tcg_env, cpu_fr[rb], tmp64);
    gen_fp_exc_raise(rc);
}
                             ...
                             uint8_t rb, uint8_t fn)
{
    ...
    case 0x08:
        /* FSQRTS */
        gen_helper_fsqrts(cpu_fr[rc], tcg_env, cpu_fr[rb]);
        break;
    case 0x09:
        /* FSQRTD */
        gen_helper_fsqrt(cpu_fr[rc], tcg_env, cpu_fr[rb]);
        break;
    case 0x10:
        /* FCMPEQ */
    ...
    case 0x20:
        /* FCVTSD */
        gen_helper_fcvtsd(cpu_fr[rc], tcg_env, cpu_fr[rb]);
        break;
    case 0x21:
        /* FCVTDS */
        gen_helper_fcvtds(cpu_fr[rc], tcg_env, cpu_fr[rb]);
        break;
    case 0x22:
        /* FCVTDL_G */
        gen_fcvtdl(rb, rc, 0);
    ...
    case 0x27:
        /* FCVTDL */
        gen_helper_fcvtdl_dyn(cpu_fr[rc], tcg_env, cpu_fr[rb]);
        break;
    case 0x28:
        /* FCVTWL */
        gen_helper_fcvtwl(cpu_fr[rc], tcg_env, cpu_fr[rb]);
        tcg_gen_ext32s_i64(cpu_fr[rc], cpu_fr[rc]);
        break;
    case 0x29:
        /* FCVTLW */
        gen_helper_fcvtlw(cpu_fr[rc], tcg_env, cpu_fr[rb]);
        break;
    case 0x2d:
        /* FCVTLS */
        gen_helper_fcvtls(cpu_fr[rc], tcg_env, cpu_fr[rb]);
        break;
    case 0x2f:
        /* FCVTLD */
        gen_helper_fcvtld(cpu_fr[rc], tcg_env, cpu_fr[rb]);
        break;
    case 0x30:
        /* FCPYS */
        tmp64 = tcg_temp_new();
    ...
    case 0x50:
        /* RFPCR */
        gen_helper_load_fpcr(cpu_fr[ra], tcg_env);
        break;
    case 0x51:
        /* WFPCR */
        gen_helper_store_fpcr(tcg_env, cpu_fr[ra]);
        break;
    case 0x54:
        /* SETFPEC0 */
        tmp64 = tcg_constant_i64(0);
        gen_helper_setfpcrx(tcg_env, tmp64);
        break;
    case 0x55:
        /* SETFPEC1 */
        tmp64 = tcg_constant_i64(1);
        gen_helper_setfpcrx(tcg_env, tmp64);
        break;
    case 0x56:
        /* SETFPEC2 */
        tmp64 = tcg_constant_i64(2);
        gen_helper_setfpcrx(tcg_env, tmp64);
        break;
    case 0x57:
        /* SETFPEC3 */
        tmp64 = tcg_constant_i64(3);
        gen_helper_setfpcrx(tcg_env, tmp64);
        break;
    case 0x58:
        /* TODO:FRECS */
                             ...
                             uint8_t rb, uint8_t rc, uint8_t fn)
{
    ...
    case 0x00:
        /* FMAS */
        gen_helper_fmas(vd, tcg_env, va, vb, vc);
        break;
    case 0x01:
        /* FMAD */
        gen_helper_fmad(vd, tcg_env, va, vb, vc);
        break;
    case 0x02:
        /* FMSS */
        gen_helper_fmss(vd, tcg_env, va, vb, vc);
        break;
    case 0x03:
        /* FMSD */
        gen_helper_fmsd(vd, tcg_env, va, vb, vc);
        break;
    case 0x04:
        /* FNMAS */
        gen_helper_fnmas(vd, tcg_env, va, vb, vc);
        break;
    case 0x05:
        /* FNMAD */
        gen_helper_fnmad(vd, tcg_env, va, vb, vc);
        break;
    case 0x06:
        /* FNMSS */
        gen_helper_fnmss(vd, tcg_env, va, vb, vc);
        break;
    case 0x07:
        /* FNMSD */
        gen_helper_fnmsd(vd, tcg_env, va, vb, vc);
        break;
    case 0x10:
        /* FSELEQ */
        tmp64 = tcg_temp_new();
        gen_helper_fcmpeq(tmp64, tcg_env, va, zero);
    ...
    case 0x12:
        /* FSELLT */
        tmp64 = tcg_temp_new();
        gen_helper_fcmplt(tmp64, tcg_env, va, zero);
        tcg_gen_movcond_i64(TCG_COND_EQ, vd, tmp64, zero, vc, vb);
        break;
    case 0x13:
        /* FSELLE */
        tmp64 = tcg_temp_new();
        gen_helper_fcmple(tmp64, tcg_env, va, zero);
        tcg_gen_movcond_i64(TCG_COND_EQ, vd, tmp64, zero, vc, vb);
        break;
    case 0x14:
        /* FSELGT */
        tmp64 = tcg_temp_new();
        gen_helper_fcmpgt(tmp64, tcg_env, va, zero);
        tcg_gen_movcond_i64(TCG_COND_NE, vd, tmp64, zero, vb, vc);
        break;
    case 0x15:
        /* FSELGE */
        tmp64 = tcg_temp_new();
        gen_helper_fcmpge(tmp64, tcg_env, va, zero);
        tcg_gen_movcond_i64(TCG_COND_NE, vd, tmp64, zero, vb, vc);
        break;
    default:
        fprintf(stderr, "Illegal insn func[%x]\n", fn);
...
{
    tcg_gen_qemu_ld_i64(t0, t1, flags, MO_LEUQ);
    tcg_gen_mov_i64(cpu_lock_addr, t1);
#ifdef SW64_FIXLOCK
...
{
    TCGv_i64 tmp = tcg_constant_i64(idx);
    gen_helper_write_csr(tcg_env, tmp, va);
}
...
static DisasJumpType gen_sys_call(DisasContext *ctx, int syscode)
{
    if (syscode >= 0x80 && syscode <= 0xbf) {
        ...
        case 0x9E:
            /* RDUNIQUE */
            tcg_gen_ld_i64(ctx->ir[IDX_V0], tcg_env,
                           offsetof(CPUSW64State, unique));
            break;
        case 0x9F:
            /* WRUNIQUE */
            tcg_gen_st_i64(ctx->ir[IDX_A0], tcg_env,
                           offsetof(CPUSW64State, unique));
            write_csr(0xc7, ctx->ir[IDX_A0], ctx->env);
            break;
#endif
        default:
            goto do_sys_call;
...
{
    TCGv_i64 tmp = tcg_constant_i64(idx);
    gen_helper_read_csr(va, tcg_env, tmp);
}
                           ...
                           bool bype)
{
    ...
    if (bype == 0) {
        ...
        tcg_gen_atomic_xchg_i64(va, addr, t1, ctx->mem_idx, MO_TEUQ);
    }
...
static void tcg_gen_srlow_i64(int ra, int rc, int rb)
{
    ...
    va = tcg_constant_i64(ra);
    vc = tcg_constant_i64(rc);
    shift = tcg_temp_new();
    vb = cpu_fr[rb];
    tcg_gen_shri_i64(shift, vb, 29);
    tcg_gen_andi_i64(shift, shift, 0xff);

    gen_helper_srlow(tcg_env, va, vc, shift);
...
static void tcg_gen_sllow_i64(int ra, int rc, int rb)
{
    ...
    gen_helper_sllow(tcg_env, va, vc, shift);
...
static void tcg_gen_sraow_i64(int ra, int rc, int rb)
{
    ...
    gen_helper_sraow(tcg_env, va, vc, shift);
...
static void gen_qemu_vstd_uh(int t0, TCGv t1, int memidx)
{
    ...
    for (i = 3; i >= 0; i--) {
        tcg_gen_movcond_i64(TCG_COND_GEU, t1, t1, addr_start, t1, addr_start);
        tcg_gen_qemu_ld_i64(tmp[i], t1, memidx, MO_TEUQ);
    ...
    for (i = 0; i < 4; i++) {
        tcg_gen_movcond_i64(TCG_COND_GEU, t1, addr_end, addr_start, addr_end, addr_start);
        tcg_gen_qemu_st_i64(tmp[i], t1, memidx, MO_TEUQ);
                               ...
                               int rc, int rd, int fn6)
{
    TCGv zz;
    TCGv args, vd;
    zz = tcg_constant_i64(((opc & 0x3) << 6) | fn6);
    args = tcg_constant_i64((ra << 16) | (rb << 8) | rc);
    vd = tcg_constant_i64(rd);

    gen_helper_vlogzz(tcg_env, args, vd, zz);
                                   ...
                                   CPUState *cpu)
{
    ...
    case 0x06:
        ...
        case 0x07:
            ...
            gen_helper_cpustate_update(tcg_env, va);
        ...
        case 0x1A:
            ...
            case 0x19:
                /* CTLZOW */
                va = tcg_constant_i64(ra);
                gen_helper_ctlzow(cpu_fr[rc], tcg_env, va);
                break;
            case 0x1A:
                /* VSLLL */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
            ...
            case 0x1E:
                ...
                gen_helper_vmaxb(tcg_env, va, vb, vc);
            ...
            case 0x1F:
                ...
                gen_helper_vminb(tcg_env, va, vb, vc);
            ...
            case 0x40:
                ...
                vb = tcg_constant_i64(rb);
                vc = tcg_constant_i64(rc);
                gen_helper_vucaddw(tcg_env, va, vb, vc);
                break;
            case 0x60:
                /* VUCADDW */
                va = tcg_constant_i64(ra);
                vb = tcg_constant_i64(disp8);
                vc = tcg_constant_i64(rc);
                gen_helper_vucaddwi(tcg_env, va, vb, vc);
                break;
            case 0x41:
                /* VUCSUBW */
                va = tcg_constant_i64(ra);
                vb = tcg_constant_i64(rb);
                vc = tcg_constant_i64(rc);
                gen_helper_vucsubw(tcg_env, va, vb, vc);
                break;
            case 0x61:
                /* VUCSUBW */
                va = tcg_constant_i64(ra);
                vb = tcg_constant_i64(disp8);
                vc = tcg_constant_i64(rc);
                gen_helper_vucsubwi(tcg_env, va, vb, vc);
                break;
            case 0x42:
                /* VUCADDH */
                va = tcg_constant_i64(ra);
                vb = tcg_constant_i64(rb);
                vc = tcg_constant_i64(rc);
                gen_helper_vucaddh(tcg_env, va, vb, vc);
                break;
            case 0x62:
                /* VUCADDH */
                va = tcg_constant_i64(ra);
                vb = tcg_constant_i64(disp8);
                vc = tcg_constant_i64(rc);
                gen_helper_vucaddhi(tcg_env, va, vb, vc);
                break;
            case 0x43:
                /* VUCSUBH */
                va = tcg_constant_i64(ra);
                vb = tcg_constant_i64(rb);
                vc = tcg_constant_i64(rc);
                gen_helper_vucsubh(tcg_env, va, vb, vc);
                break;
            case 0x63:
                /* VUCSUBH */
                va = tcg_constant_i64(ra);
                vb = tcg_constant_i64(disp8);
                vc = tcg_constant_i64(rc);
                gen_helper_vucsubhi(tcg_env, va, vb, vc);
                break;
            case 0x44:
                /* VUCADDB */
                va = tcg_constant_i64(ra);
                vb = tcg_constant_i64(rb);
                vc = tcg_constant_i64(rc);
                gen_helper_vucaddb(tcg_env, va, vb, vc);
                break;
            case 0x64:
                /* VUCADDB */
                va = tcg_constant_i64(ra);
                vb = tcg_constant_i64(disp8);
                vc = tcg_constant_i64(rc);
                gen_helper_vucaddbi(tcg_env, va, vb, vc);
                break;
            case 0x45:
                /* VUCSUBB */
                va = tcg_constant_i64(ra);
                vb = tcg_constant_i64(rb);
                vc = tcg_constant_i64(rc);
                gen_helper_vucsubb(tcg_env, va, vb, vc);
                break;
            case 0x65:
                /* VUCSUBB */
                va = tcg_constant_i64(ra);
                vb = tcg_constant_i64(disp8);
                vc = tcg_constant_i64(rc);
                gen_helper_vucsubbi(tcg_env, va, vb, vc);
                break;
            case 0x46:
                /* SRAOW */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
            ...
            case 0x49:
                ...
                vb = tcg_constant_i64(rb);
                vc = tcg_constant_i64(rc);
                gen_helper_vsm4r(tcg_env, va, vb, vc);
                break;
            case 0x4A:
                /* VBINVW */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
            ...
            case 0x4B:
                ...
                vb = tcg_constant_i64(rb);
                vc = tcg_constant_i64(rc);
                gen_helper_vcmpueqb(tcg_env, va, vb, vc);
                break;
            case 0x6B:
                /* VCMPUEQBI*/
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                ...
                vb = tcg_constant_i64(disp8);
                vc = tcg_constant_i64(rc);
                gen_helper_vcmpueqbi(tcg_env, va, vb, vc);
                break;
            case 0x4C:
                /* VCMPUGTB */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                ...
                vb = tcg_constant_i64(rb);
                vc = tcg_constant_i64(rc);
                gen_helper_vcmpugtb(tcg_env, va, vb, vc);
                break;
            case 0x6C:
                /* VCMPUGTBI */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                ...
                vb = tcg_constant_i64(disp8);
                vc = tcg_constant_i64(rc);
                gen_helper_vcmpugtbi(tcg_env, va, vb, vc);
                break;
            case 0x4D:
                /* VSM3MSW */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                ...
                vb = tcg_constant_i64(rb);
                vc = tcg_constant_i64(rc);
                gen_helper_vsm3msw(tcg_env, va, vb, vc);
                break;
            case 0x50:
                /* VMAXH */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                ...
                gen_helper_vmaxh(tcg_env, va, vb, vc);
            ...
            case 0x51:
                ...
                gen_helper_vminh(tcg_env, va, vb, vc);
            ...
            case 0x52:
                ...
                gen_helper_vmaxw(tcg_env, va, vb, vc);
            ...
            case 0x53:
                ...
                gen_helper_vminw(tcg_env, va, vb, vc);
            ...
            case 0x56:
                ...
                gen_helper_vumaxb(tcg_env, va, vb, vc);
            ...
            case 0x57:
                ...
                gen_helper_vuminb(tcg_env, va, vb, vc);
            ...
            case 0x58:
                ...
                gen_helper_vumaxh(tcg_env, va, vb, vc);
            ...
            case 0x59:
                ...
                gen_helper_vuminh(tcg_env, va, vb, vc);
            ...
            case 0x5A:
                ...
                gen_helper_vumaxw(tcg_env, va, vb, vc);
            ...
            case 0x5B:
                ...
                gen_helper_vuminw(tcg_env, va, vb, vc);
            ...
            case 0x68:
                ...
                vb = tcg_constant_i64(disp8);
                vc = tcg_constant_i64(rc);
                gen_helper_vsm4key(tcg_env, va, vb, vc);
                break;
            case 0x80:
                /* VADDS */
                for (i = 0; i < 128; i += 32)
            ...
            case 0x88:
                /* VSQRTS */
                for (i = 0; i < 128; i += 32)
                    gen_helper_fsqrts(cpu_fr[rc + i], tcg_env,
                                      cpu_fr[rb + i]);
                break;
            case 0x89:
                /* VSQRTD */
                for (i = 0; i < 128; i += 32)
                    gen_helper_fsqrt(cpu_fr[rc + i], tcg_env,
                                     cpu_fr[rb + i]);
                break;
            case 0x8C:
                /* VFCMPEQ */
            ...
            case 0x95:
                ...
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                for (i = 0; i < 128; i += 32) {
                    gen_helper_fcvtsd(cpu_fr[rc + i], tcg_env,
                                      cpu_fr[rb + i]);
                }
                break;
            case 0x96:
                ...
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                for (i = 0; i < 128; i += 32) {
                    gen_helper_fcvtds(cpu_fr[rc + i], tcg_env,
                                      cpu_fr[rb + i]);
                }
                break;
            case 0x99:
                ...
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                for (i = 0; i < 128; i += 32) {
                    gen_helper_fcvtls(cpu_fr[rc + i], tcg_env,
                                      cpu_fr[rb + i]);
                }
                break;
            case 0x9A:
                ...
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                for (i = 0; i < 128; i += 32) {
                    gen_helper_fcvtld(cpu_fr[rc + i], tcg_env,
                                      cpu_fr[rb + i]);
                }
                break;
            case 0x9B:
                ...
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                for (i = 0; i < 128; i += 32) {
                    gen_helper_fcvtdl_dyn(cpu_fr[rc + i], tcg_env,
                        cpu_fr[rb + i]);
                }
                break;
            case 0x9C:
            ...
            case 0xAC:
                ...
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                tmp64 = tcg_temp_new();
                tmp64_0 = tcg_constant_i64(0);
                for (i = 0; i < 128; i += 32) {
                    va = cpu_fr[ra + i];
                    vb = cpu_fr[rb + i];
                    vc = cpu_fr[rc + i];
                    gen_helper_fcmpge_s(tmp64, tcg_env, va, vb);
            ...
            case 0xAD:
                ...
                for (i = 0; i < 128; i += 32) {
                    ...
                    vb = cpu_fr[rb + i];
                    vc = cpu_fr[rc + i];
                    gen_helper_fcmple_s(tmp64, tcg_env, va, vb);
                    tcg_gen_movcond_i64(TCG_COND_NE, vc, tmp64, tmp64_0,
                                        va, vb);
                }
                break;
            case 0xAE:
                ...
                for (i = 0; i < 128; i += 32) {
                    ...
                    vb = cpu_fr[rb + i];
                    vc = cpu_fr[rc + i];
                    gen_helper_fcmpge(tmp64, tcg_env, va, vb);
                    tcg_gen_movcond_i64(TCG_COND_NE, vc, tmp64, tmp64_0,
                                        va, vb);
                }
                break;
            case 0xAF:
                ...
                for (i = 0; i < 128; i += 32) {
                    ...
                    vb = cpu_fr[rb + i];
                    vc = cpu_fr[rc + i];
                    gen_helper_fcmple(tmp64, tcg_env, va, vb);
                    tcg_gen_movcond_i64(TCG_COND_NE, vc, tmp64, tmp64_0,
                                        va, vb);
                }
                break;
        ...
        case 0x1B:
            ...
            case 0x00:
                /* VMAS */
                for (i = 0; i < 128; i += 32)
                    gen_helper_fmas(cpu_fr[rc + i], tcg_env, cpu_fr[ra + i],
                                    cpu_fr[rb + i], cpu_fr[rd + i]);
                break;
            case 0x01:
                /* VMAD */
                for (i = 0; i < 128; i += 32)
                    gen_helper_fmad(cpu_fr[rc + i], tcg_env, cpu_fr[ra + i],
                                    cpu_fr[rb + i], cpu_fr[rd + i]);
                break;
            case 0x02:
                /* VMSS */
                for (i = 0; i < 128; i += 32)
                    gen_helper_fmss(cpu_fr[rc + i], tcg_env, cpu_fr[ra + i],
                                    cpu_fr[rb + i], cpu_fr[rd + i]);
                break;
            case 0x03:
                /* VMSD */
                for (i = 0; i < 128; i += 32)
                    gen_helper_fmsd(cpu_fr[rc + i], tcg_env, cpu_fr[ra + i],
                                    cpu_fr[rb + i], cpu_fr[rd + i]);
                break;
            case 0x04:
                /* VNMAS */
                for (i = 0; i < 128; i += 32)
                    gen_helper_fnmas(cpu_fr[rc + i], tcg_env,
            ...
            case 0x05:
                /* VNMAD */
                for (i = 0; i < 128; i += 32)
                    gen_helper_fnmad(cpu_fr[rc + i], tcg_env,
                                     cpu_fr[ra + i], cpu_fr[rb + i],
                                     cpu_fr[rd + i]);
                break;
            case 0x06:
                /* VNMSS */
                for (i = 0; i < 128; i += 32)
                    gen_helper_fnmss(cpu_fr[rc + i], tcg_env,
                                     cpu_fr[ra + i], cpu_fr[rb + i],
                                     cpu_fr[rd + i]);
                break;
            case 0x07:
                /* VNMSD */
                for (i = 0; i < 128; i += 32)
                    gen_helper_fnmsd(cpu_fr[rc + i], tcg_env,
                                     cpu_fr[ra + i], cpu_fr[rb + i],
                                     cpu_fr[rd + i]);
                break;
            case 0x10:
                ...
                tmp64_0 = tcg_constant_i64(0);
                for (i = 0; i < 128; i += 32) {
                    gen_helper_fcmpeq(tmp64, tcg_env, cpu_fr[ra + i],
                                      tmp64_0);
            ...
            case 0x26:
                /* VCONW */
                tmp64 = tcg_constant_i64(ra << 8 | rb);
                tmp64_0 = tcg_temp_new();
                vd = tcg_constant_i64(rc);
                tcg_gen_shri_i64(tmp64_0, cpu_fr[rd], 2);
                tcg_gen_andi_i64(tmp64_0, tmp64_0, 0x7ul);
                gen_helper_vconw(tcg_env, tmp64, vd, tmp64_0);
                break;
            case 0x27:
                /* VSHFW */
                tmp64 = tcg_constant_i64(ra << 8 | rb);
                vd = tcg_constant_i64(rc);
                gen_helper_vshfw(tcg_env, tmp64, vd, cpu_fr[rd]);
                break;
            case 0x28:
                /* VCONS */
                tmp64 = tcg_constant_i64(ra << 8 | rb);
                ...
                gen_helper_vcond(tcg_env, tmp64, vd, tmp64_0);
                break;
            case 0x29:
            ...
            case 0x2A:
                ...
                va = cpu_fr[ra];
                vb = tcg_constant_i64(rb);
                vc = tcg_constant_i64(rd);
                vd = tcg_constant_i64(rc);
                gen_helper_vinsb(tcg_env, va, vb, vc, vd);
                break;
            case 0x2B:
                /* VINSH */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                ...
                vc = tcg_constant_i64(rd);
                vd = tcg_constant_i64(rc);
                gen_helper_vinsh(tcg_env, va, vb, vc, vd);
                break;
            case 0x2C:
                /* VINSECTLH */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                ...
                vb = tcg_constant_i64(rb);
                vd = tcg_constant_i64(rc);
                gen_helper_vinsectlh(tcg_env, va, vb, vd);
                break;
            case 0x2D:
                /* VINSECTLW */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                ...
                vb = tcg_constant_i64(rb);
                vd = tcg_constant_i64(rc);
                gen_helper_vinsectlw(tcg_env, va, vb, vd);
                break;
            case 0x2E:
                /* VINSECTLL */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
            ...
            case 0x2F:
                ...
                vb = tcg_constant_i64(rb);
                vd = tcg_constant_i64(rc);
                gen_helper_vinsectlb(tcg_env, va, vb, vd);
                break;
            case 0x30:
                /* VSHFQ */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                ...
                vc = tcg_constant_i64(rd);
                vd = tcg_constant_i64(rc);
                gen_helper_vshfq(tcg_env, va, vb, vc, vd);
                break;
            case 0x31:
                /* VSHFQB */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                ...
                vb = tcg_constant_i64(rb);
                vd = tcg_constant_i64(rc);
                gen_helper_vshfqb(tcg_env, va, vb, vd);
                break;
            case 0x32:
                /* VCPYB */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
            ...
            case 0x34:
                ...
                vc = tcg_constant_i64(rd);
                vd = tcg_constant_i64(rc);
                gen_helper_vsm3r(tcg_env, va, vb, vc, vd);
                break;
            case 0x35:
                /* VFCVTSH */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                ...
                vc = tcg_constant_i64(rd);
                vd = tcg_constant_i64(rc);
                gen_helper_vfcvtsh(tcg_env, va, vb, vc, vd);
                break;
            case 0x36:
                /* VFCVTHS */
                arch_assert(test_feature(ctx->env, SW64_FEATURE_CORE4));
                ...
                vc = tcg_constant_i64(rd);
                vd = tcg_constant_i64(rc);
                gen_helper_vfcvths(tcg_env, va, vb, vc, vd);
                break;
            default:
                printf("ILLEGAL BELOW OPC[%x] func[%08x]\n", opc, fn6);
                ret = gen_invalid(ctx);
        ...
        case 0x24:
            /* LDL_U */
            gen_load_mem(ctx, &tcg_gen_qemu_ld64, ra, rb, disp16, 0, ~0x7UL);
            break;
        case 0x25:
            /* PRI_LD */
#ifndef CONFIG_USER_ONLY
            if ((insn >> 12) & 1) {
                gen_load_mem(ctx, &gen_qemu_pri_ldl, ra, rb, disp12, 0, ~0x7UL);
            } else {
                gen_load_mem(ctx, &gen_qemu_pri_ldw, ra, rb, disp12, 0, ~0x3UL);
            }
#endif
            break;
        case 0x26:
        ...
        case 0x2c:
            /* STL_U */
            gen_store_mem(ctx, &tcg_gen_qemu_st64, ra, rb, disp16, 0, ~0x7UL);
            break;
        case 0x2d:
            /* PRI_ST */
#ifndef CONFIG_USER_ONLY
            if ((insn >> 12) & 1) {
                gen_store_mem(ctx, &gen_qemu_pri_stl, ra, rb, disp12, 0, ~0x7UL);
            } else {
                gen_store_mem(ctx, &gen_qemu_pri_stw, ra, rb, disp12, 0, ~0x3UL);
            }
#endif
            break;
        case 0x2e:
...
static void sw64_tr_disas_log(const DisasContextBase* dcbase,
		              CPUState* cpu, FILE *logfile)
{
    ...
    target_disas(logfile, cpu, dcbase->pc_first & (~0x3UL), dcbase->tb->size);
}
		           ...
		           target_ulong pc, void *host_pc)
{
    ...
    translator_loop(cpu, tb, max_insns, pc, host_pc, &sw64_trans_ops, &dc.base);
}
