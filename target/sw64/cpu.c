...
static void sw64_restore_state_to_opc(CPUState *cs,
...
#ifndef CONFIG_USER_ONLY
    SW64CPU *cpu = SW64_CPU(dev);
    MachineState *ms = MACHINE(qdev_get_machine());
    unsigned int max_cpus = ms->smp.max_cpus;

    if (cpu->core_id > 0) {
        cpu_hot_id = cpu->core_id;
        if (!cpu_hot_id || cpu_hot_id > max_cpus - 1) {
            error_setg(&local_err, "error:"
                "Cpu num must be less than max cpus and can not be 0!");
...
static void sw64_cpu_unrealizefn(DeviceState *dev)
{
    SW64CPUClass *scc = SW64_CPU_GET_CLASS(dev);
#ifndef CONFIG_USER_ONLY
    CPUState *cs = CPU(dev);
    cpu_remove_sync(CPU(dev));
    qemu_unregister_reset(sw64_machine_cpu_reset, cs);
#endif
...
bool sw64_cpu_has_work(CPUState *cs)
{
    ...
    return cs->interrupt_request & (CPU_INTERRUPT_HARD | CPU_INTERRUPT_TIMER |
                                    CPU_INTERRUPT_II0 | CPU_INTERRUPT_MCHK | CPU_INTERRUPT_IINM);
}
...
static Property sw64_cpu_properties[] = {
    ...
    DEFINE_PROP_UINT32("thread-id", SW64CPU, thread_id, 0),
    DEFINE_PROP_UINT32("core-id", SW64CPU, core_id, 0),
    DEFINE_PROP_UINT32("socket-id", SW64CPU, socket_id, 0),
#else
    DEFINE_PROP_UINT32("thread-id", SW64CPU, thread_id, 0),
    DEFINE_PROP_UINT32("core-id", SW64CPU, core_id, 0xFFFFFFFF),
    DEFINE_PROP_UINT32("socket-id", SW64CPU, socket_id, 0),
#endif
    DEFINE_PROP_UINT32("node-id", SW64CPU, node_id, -1),
...
static const struct TCGCPUOps sw64_tcg_ops = {
    ...
    .restore_state_to_opc = sw64_restore_state_to_opc,
...
static void sw64_cpu_class_init(ObjectClass *oc, void *data)
{
    ...
    scc->parent_unrealize = dc->unrealize;
    dc->unrealize = sw64_cpu_unrealizefn;
